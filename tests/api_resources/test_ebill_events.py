...
class TestEbillEvents(object):
    @staticmethod
    def _construct_resource():
        access_token = "test_access_token"
        organisation_id = "testxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxx1"

        return pingen2sdk.EbillEvents(organisation_id, access_token)

    @responses.activate
    def test_get_collection(self):
        ebill_id = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxx1"
        url = (
            "%s/organisations/testxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxx1/deliveries/ebills/%s/events"
            % (pingen2sdk.api_production, ebill_id)
        )

        ebill_events = self._construct_resource()
        ...
        response = ebill_events.get_collection(
